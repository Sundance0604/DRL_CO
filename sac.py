...
class MultiAgentAC(torch.nn.Module):
    def __init__(self, device, VEHICLE_STATE_DIM, 
        ...
        self.target_critic_1 = ValueNet(STATE_DIM, HIDDEN_DIM).to(device)
        self.target_critic_2 = ValueNet(STATE_DIM, HIDDEN_DIM).to(device)   
        self.target_critic_1.load_state_dict(self.critic_1.state_dict())
        self.target_critic_2.load_state_dict(self.critic_2.state_dict())
        
        # 优化器
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=0.001)
        self.critic_1_optimizer = optim.Adam(self.critic_1.parameters(), lr=0.001)
        self.critic_2_optimizer = optim.Adam(self.critic_2.parameters(), lr=0.001)

        # 超参数
        self.log_alpha = torch.tensor(np.log(0.01), dtype=torch.float)
        self.log_alpha.requires_grad = True
        self.log_alpha_optimizer = optim.Adam([self.log_alpha], lr=0.001)
        self.target_entropy = np.log(NUM_CITIES)  # 目标熵值，此处设置与动手强书中不一样
        # 动态智能体管理 ⭐
    ...
    def calculate_target_q(self, vehicle_states, order_states, mask):
        with torch.no_grad():
            ...
            # 计算两个Critic的Q值
            q1_values = self.target_critic_1(self._get_global_state(v_encoded, o_encoded))
            q2_values = self.target_critic_2(self._get_global_state(v_encoded, o_encoded))
            min_q_values = torch.min(q1_values, q2_values)

            # 计算期望Q值
            expected_q = torch.sum(probs * (min_q_values - torch.exp(self.log_alpha) * log_probs), dim=-1)
    ...
    def update_ppo(self, time, ppo_eposide=30):
        ...
        for _ in range(ppo_eposide):
            ...
            self.policy_loss.append(actor_loss.item())
            self.value_loss.append(critic_loss.item())
            # 更新Actor
            self.actor_optimizer.zero_grad()
